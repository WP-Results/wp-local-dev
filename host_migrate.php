 ...
 */
function icit_srdb_define_find( $filename = 'wp-local-config.php' ) {
...
// Check that we need to scan wp-local-config.
...
// If we're at the start we'll check to see if wp is about so we can get the details from the wp-local-config.
if ( $step == 0 || $step == 1 )
	$step = file_exists( dirname( __FILE__ ) . '/wp-local-config.php' ) ? 1 : 2;

// Scan wp-local-config for the defines. We can't just include it as it will try and load the whole of wordpress.
if ( $loadwp && file_exists( dirname( __FILE__ ) . '/wp-local-config.php' ) )
	list( $host, $data, $user, $pass, $char ) = icit_srdb_define_find( 'wp-local-config.php' );
...
switch ( $step ) {
	case 1:
		...
		<form action="<?php icit_srdb_form_action( ); ?>" method="post">
			<fieldset>
				<p><label for="loadwp"><input type="checkbox" checked="checked" value="1" name="loadwp" id="loadwp" /> Pre-populate the DB values form with the ones used in wp-local-config?  It is possible to edit them later.</label></p> <?php
...
*/ ?>
		<div class="help">
			...
			You then, if required, fix up your .htaccess and wp-local-config.php appropriately.  Once
